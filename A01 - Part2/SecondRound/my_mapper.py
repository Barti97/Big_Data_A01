...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for i in my_input_stream:
        ...
        day = get_day_of_week(data[4].split(' ')[0])
        hour = data[4].split(' ')[1].split(':')[0] #15-10-2018 15:32:30
        ...
        if status == 0:
            if stationName == my_mapper_input_parameters[0]:
                if bikesAvailable == 0:
                    if key not in myTotals:
                        myTotals[key] = 1
                    else:
                        myTotals[key] = myTotals[key] + 1

    for t in myTotals:
        my_output_stream.write( t +'\t(' + str(myTotals[t]) + ')\n')
