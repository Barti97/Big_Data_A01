...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for i in my_input_stream:
        ...
        status = int(data[0])
        stationName = str(data[1])
        time = data[4].split(' ')[1] #15-10-2018 15:32:30
        year = data[4].split(' ')[0].split("-")[2]
        month = data[4].split(' ')[0].split("-")[1]
        day = data[4].split(' ')[0].split("-")[0]
        date = year+"-"+month+"-"+day
        bikesAvailable = int(data[5])
        
        result = date, time
        
        if status == 0:
            if stationName == my_mapper_input_parameters[0]:
                if bikesAvailable == 0:
                    myTotals.append(result)

    for t in myTotals:
        my_output_stream.write( str(t[0]) + '\t(' + str(t[1]) + ')\n')
